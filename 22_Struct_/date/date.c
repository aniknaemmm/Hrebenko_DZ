...
void enterDate(Date *data){
    ...
    }while(!(chekNormalyEnterDate(*data))&&printf("ivalid value%d",feb));
...
int diffDateDay(Date firsdata,Date nextdata){
    ...
    while((nextdata.month!=firsdata.month)||(nextdata.day!=firsdata.day)||(nextdata.year!=firsdata.year)){
        ...
        if(nextdata.day==0){
               nextdata.month-=1;
            if((nextdata.month==feb)&&visokYear(nextdata))
                    nextdata.day=29;
            else if((nextdata.month==feb)&&!visokYear(nextdata))
                    nextdata.day=28;
            else if((nextdata.month==aprel)||(nextdata.month==iyun)||(nextdata.month==sentiab)||(nextdata.month==novemb))
                    nextdata.day=30;
